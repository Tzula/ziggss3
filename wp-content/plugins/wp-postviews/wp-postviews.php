...
function get_timespan_most_viewed($mode = '', $limit = 10, $days = 7, $display = true) {
	...
	if($most_viewed) {
		...
		foreach ($hots as $key => $hot) {
			...
			$temp .= '<li style="list-style-type:none;" class="asidepost-list-li"><div class="hot_post"><a href="'.get_permalink().'" class="hot_imginfo">'.$thumb.'</a><div class="hot_post_info"><a class="hot_subtitle" href="'.get_permalink().'" style="font-family: sans-serif;">'.mb_strimwidth($hot['post_title'],0,56,'').'</a>'.__('', 'wp-postviews').'</div></div></li>';
